...
const auth = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      return res.status(200).json({ items: { roles: [], userId: "" }, message: `User is unauthorized` });
    }
    ...
    res.status(200).json({ items: { roles: roles, userId: user.id, username: user.username, email: user.email }, message: `User ${user.username} is authorized` });
  }
  catch (error) {
...
const getUserArticles = async (req: Request, res: Response) => {
  ...
  let articlesApprovable = articles.map((article) => ({
    id: article.id,
  ...
  if (user.userRoles.some(role => role.name === RoleRecordTypeEnumeration[1])) {
    articlesApprovable = articlesApprovable.map((article) => ({
      ...article,
