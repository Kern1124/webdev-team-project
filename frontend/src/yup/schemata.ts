...
export const ArticleCreateSchema = yup.object().shape({
  heading: yup
    .string()
    .min(6, "Article heading has to be at least 6 characters long"),
  contents: yup.string().required("Content cannot be empty"),
  categories: yup
    .array()
    .of(yup.string().required())
    .min(1, "A category must be selected")
    .required("A category must be selected"),
  newspaperId: yup.string().required("Newspaper must be selected"),
});
